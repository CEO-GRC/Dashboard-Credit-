...
TRANSLATIONS = {
    'en': {
        ...
        'credit_limit_total': 'Credit Limit',
        'credit_usage_pct': '% Credit Used',
        'invoices': 'invoices',
    ...
    'es': {
        ...
        'credit_limit_total': 'Límite de Crédito',
        'credit_usage_pct': '% Crédito Usado',
        'invoices': 'facturas',
...
def load_and_process_data(uploaded_file):
    ...
    try:
        ...
        # CRITICAL: Extract Credit Limits from subtotal rows BEFORE removing them
        # Subtotal rows have 'Result' in Document date and contain the Credit Limit
        subtotal_rows = df[df['Document date'] == 'Result'].copy()
        
        # Create a mapping of Counterparty Name -> Credit Limit from subtotals
        credit_limit_map = {}
        if not subtotal_rows.empty and 'Counterparty Name' in subtotal_rows.columns and 'Credit Limit' in subtotal_rows.columns:
            for _, row in subtotal_rows.iterrows():
                customer = row['Counterparty Name']
                credit_limit = row['Credit Limit']
                # Convert to numeric if it's not already
                if pd.notna(credit_limit):
                    try:
                        credit_limit_value = float(str(credit_limit).replace(',', ''))
                        if credit_limit_value > 0:
                            credit_limit_map[customer] = credit_limit_value
                    except:
                        pass
        
        # Now remove subtotal rows
        ...
        # Apply the Credit Limit from the mapping to all rows of each customer
        if credit_limit_map and 'Counterparty Name' in df.columns:
            df['Credit Limit'] = df['Counterparty Name'].map(credit_limit_map).fillna(0)
...
def calculate_kpis(df):
    ...
    # Calculate total credit limit (sum of unique credit limits per customer)
    kpis['total_credit_limit'] = client_totals[client_totals['Credit Limit'] > 0]['Credit Limit'].sum()
    
    # Calculate credit usage percentage
    total_debt_for_clients_with_limit = client_totals[client_totals['Credit Limit'] > 0]['Open Amount'].sum()
    kpis['credit_usage_pct'] = (total_debt_for_clients_with_limit / kpis['total_credit_limit'] * 100) if kpis['total_credit_limit'] > 0 else 0
...
def create_aging_chart(df, theme='light'):
    ...
    fig.add_trace(go.Bar(
        ...
        textfont=dict(size=10, color='#1E293B' if theme == 'light' else '#F1F5F9'),
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=False, tickangle=-45, tickfont=dict(size=8, color=text_color)),
        yaxis=dict(showgrid=True, gridcolor=grid_color, gridwidth=0.5, tickfont=dict(color=text_color)),
        hovermode='x unified'
    )
...
def create_top_debtors_chart(df, top_n=10, theme='light'):
    ...
    fig.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor=grid_color, gridwidth=0.5, tickfont=dict(color=text_color)),
        yaxis=dict(showgrid=False, tickfont=dict(size=8, color=text_color)),
        hovermode='y unified'
    )
...
def main():
    ...
    <div class="kpi-grid">
        ...
        {kpi_card(t('credit_limit_total'), human_format(kpis['total_credit_limit']), f"{t('of')} {kpis['unique_customers']} {t('customers')}", 'neutral')}
    </div>
    """
    ...
    with col2:
        st.metric(t('credit_usage_pct'), fmt_pct(kpis['credit_usage_pct']))
    with col3:
                    ...
                    with pd.ExcelWriter(output, engine='openpyxl') as writer:
                        ...
                        kpi_summary = pd.DataFrame({
                            'KPI': [t('gross_receivables'), t('customer_credits'), t('current_pct'), t('overdue'),
                                   t('credit_limit_total'), t('credit_usage_pct'), t('total_customers'), t('invoices'), t('avg_days_overdue')],
                            'Value': [fmt_currency(kpis['gross_debt']), fmt_currency(kpis['credits']),
                                     fmt_pct(kpis['pct_current']), fmt_pct(kpis['pct_overdue']),
                                     fmt_currency(kpis['total_credit_limit']), fmt_pct(kpis['credit_usage_pct']), kpis['unique_customers'], kpis['total_invoices'],
                                     f"{int(kpis['avg_days_overdue'])} days"]
                        })
